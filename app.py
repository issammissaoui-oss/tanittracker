...
# Initialisation à None pour ne pas afficher de fausse position par défaut
ship_data = {
    "lat": None, 
    "lon": None, 
    "speed": 0.0, 
...
def on_message(ws, message):
    ...
    try:
        ...
        if msg_type == "SubscriptionConfirmation":
            ship_data["status"] = "Connecté, attente de signal AIS..."
            return
...
def on_error(ws, error):
    global ship_data
    ship_data["status"] = "Erreur réseau WebSocket"
...
def run_ais_stream():
    def on_open(ws):
        # Abonnement large et ciblé sur la zone et le MMSI
        sub = {
            "APIKey": AIS_API_KEY,
            "BoundingBoxes": [[[30.0, -6.0], [45.0, 16.0]]],
...
<body>
    <div id="panel">
        ...
        <div class="data-row"><strong>Position :</strong> <span id="pos">En attente de signal...</span></div>
    ...
    <script>
        const map = L.map('map').setView([36.817, 10.305], 6);
