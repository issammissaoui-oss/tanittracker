...
# État global du navire C/F TANIT (avec valeurs par défaut de secours si le navire est à quai)
ship_data = {
    "mmsi": TANIT_MMSI,
    "name": "C/F TANIT",
    "latitude": 36.8170,  # Position initiale de référence (Port de La Goulette / Tunis)
    "longitude": 10.3050,
    "speed": 0.0,
    "course": 0.0,
    "timestamp": "En attente de trame active...",
    "packets": 0,
    "status": "Connexion au flux en cours..."
}
...
def on_message(ws, message):
    ...
    try:
        ...
        if msg_type == "SubscriptionConfirmation":
            ship_data["status"] = "Connecté au flux AIS (En attente de signal)"
            return
        ...
        # Filtrage strict sur le MMSI du C/F TANIT
        if TANIT_MMSI in mmsi_str or "TANIT" in ship_name:
            ...
            if lat is not None and lon is not None:
                ...
                print(f"[TANIT LIVE] Position actualisée -> Lat: {lat}, Lon: {lon}")
    except Exception as e:
        print("Erreur de traitement du message:", e)

def on_error(ws, error):
    global ship_data
    ship_data["status"] = "Erreur de connexion WebSocket"
    print("Erreur WebSocket:", error)

def on_close(ws, close_status_code, close_msg):
    global ship_data
    ship_data["status"] = "Déconnecté, reconnexion automatique..."
    print(f"Fermeture WebSocket: {close_status_code} - {close_msg}")

def run_ais_stream():
    def on_open(ws):
        # Configuration de l'abonnement global sur la Méditerranée
        sub = {
            "APIKey": AIS_API_KEY,
        ...
        print("Abonnement WebSocket envoyé avec succès pour le C/F TANIT.")
    ...
    while True:
        try:
            ...
            print("Exception dans la boucle flux:", e)
        time.sleep(5)
...
<body>
    ...
    <script>
        var map = L.map('map').setView([36.8170, 10.3050], 7);
        ...
        // Actualisation automatique de l'interface toutes les 2 secondes
