...
# État global du navire C/F TANIT
ship_data = {
    "mmsi": TANIT_MMSI,
    "name": "C/F TANIT",
    "latitude": None,
    ...
    "speed": 0.0,
    "course": 0.0,
    "timestamp": "En attente du signal en direct...",
    "packets": 0,
...
def on_message(ws, message):
    ...
    try:
        ...
        if msg_type == "SubscriptionConfirmation":
            ship_data["status"] = "Connecté (En attente de trames AIS)"
            return

        meta = packet.get("MetaData", {})
        mmsi_str = str(meta.get("MMSI", ""))
        
        if TANIT_MMSI in mmsi_str:
            pos = {}
            ...
            if lat is not None and lon is not None:
                ...
                print(f"[TANIT LIVE] Lat: {lat}, Lon: {lon} | Vitesse: {ship_data['speed']} nds")
...
def run_ais_stream():
    def on_open(ws):
        # Abonnement strict ciblant le MMSI du C/F TANIT sur toute la Méditerranée
        sub = {
            "APIKey": AIS_API_KEY,
        ...
        print("Abonnement envoyé à AISStream pour le C/F TANIT.")
