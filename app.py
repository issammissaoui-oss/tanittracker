...
# Coordonnées calées précisément sur le bassin de La Goulette (selon votre repère)
ship_data = {
    "mmsi": TANIT_MMSI,
    "name": "C/F TANIT",
    "latitude": 36.8125,  
    ...
    "speed": 0.0,
    "course": 0.0,
    "timestamp": "Position au quai (En attente de trame AIS)",
    "packets": 0,
    "status": "En attente de signal radio portuaire..."
}
...
def on_message(ws, message):
    ...
    try:
        ...
        if msg_type == "SubscriptionConfirmation":
            ship_data["status"] = "Connecté au flux AIS (Recherche active)"
            return
        ...
        if TANIT_MMSI in mmsi_str or "TANIT" in ship_name:
            ...
            if lat is not None and lon is not None:
                ...
                print(f"[TANIT LIVE] Signal capté -> Lat: {lat}, Lon: {lon}")
...
def on_close(ws, close_status_code, close_msg):
    global ship_data
    ship_data["status"] = "Déconnecté, reconnexion en cours..."
...
<body>
    ...
    <script>
        var map = L.map('map').setView([36.8125, 10.3090], 14);
