...
ship_data = {
    ...
    "speed": 0.0,
    "course": 0.0,
    "timestamp": "En attente de signal direct...",
    "packets": 0,
    "status": "Connexion au flux..."
}
...
def on_message(ws, message):
    ...
    try:
        ...
        if msg_type == "SubscriptionConfirmation":
            ship_data["status"] = "Flux actif (En attente du C/F TANIT)"
            return
        ...
        if TANIT_MMSI in mmsi_str or "TANIT" in ship_name:
            ...
            if lat is not None and lon is not None:
                ...
                print(f"[TANIT LIVE] Signal reçu -> Lat: {lat}, Lon: {lon}")
...
def run_ais_stream():
    def on_open(ws):
        # OBLIGATOIRE : AISStream exige des BoundingBoxes + le filtre MMSI optionnel
        sub = {
            "APIKey": AIS_API_KEY,
            "BoundingBoxes": [[[30.0, -6.0], [45.0, 36.0]]],  # Large zone englobant la Méditerranée
            "FiltersShipMMSI": [TANIT_MMSI],
            "FilterMessageTypes": ["PositionReport", "StandardClassBPositionReport"]
        }
        ws.send(json.dumps(sub))
        print("Abonnement complet envoyé avec BoundingBoxes pour le MMSI:", TANIT_MMSI)
