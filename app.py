...
# Données dynamiques du navire (mises à jour en temps réel par le WebSocket)
ship_data = {
    "mmsi": TANIT_MMSI,
    "name": "C/F TANIT",
    "latitude": 36.8170,  
    ...
    "speed": 0.0,
    "course": 0.0,
    "timestamp": "En attente de signal AIS...",
    "packets": 0,
    "status": "Écoute du flux en cours..."
}
...
def on_message(ws, message):
    ...
    try:
        ...
        # Filtrer par le MMSI du C/F TANIT ou son nom
        if TANIT_MMSI in mmsi_str or "TANIT" in ship_name:
            ...
            if lat is not None and lon is not None and lat != 0 and lon != 0:
                ...
                ship_data["packets"] += 1
                ship_data["status"] = "En direct (Signal reçu)"
                print(f"--> [TANIT MIS À JOUR] Lat: {lat}, Lon: {lon} | Vitesse: {ship_data['speed']} nds | Cap: {ship_data['course']}°", flush=True)
...
def on_close(ws, close_status_code, close_msg):
    ...
    print("Connexion WebSocket fermée, tentative...", flush=True)

def run_ais_stream():
    def on_open(ws):
        # Boîte englobante large couvrant la Méditerranée et les routes du Tanit
        sub = {
            "APIKey": AIS_API_KEY,
        ...
        print("Abonnement envoyé pour le MMSI 672748000.", flush=True)
...
<body>
    ...
    <script>
        var map = L.map('map').setView([36.8170, 10.3050], 10);
