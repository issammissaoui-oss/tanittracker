...
# Position initiale actualisée du navire
ship_data = {
    "mmsi": TANIT_MMSI,
    "name": "C/F TANIT",
    "latitude": 36.8170,  
...
def on_message(ws, message):
    ...
    try:
        ...
        if msg_type == "SubscriptionConfirmation":
            ship_data["status"] = "Connecté (Filtre actif)"
            print("[AIS] Abonnement validé et accepté par le serveur.", flush=True)
            return
        ...
        if TANIT_MMSI in mmsi_str or "TANIT" in ship_name:
            ...
            if lat is not None and lon is not None and lat != 0 and lon != 0:
                ...
                print(f"--> [TANIT TROUVÉ] Lat: {lat}, Lon: {lon}", flush=True)
...
def on_close(ws, close_status_code, close_msg):
    ...
    print("Connexion fermée, tentative de reconnexion...", flush=True)

def run_ais_stream():
    def on_open(ws):
        # Ajout obligatoire de BoundingBoxes pour valider le filtre sur aisstream.io
        sub = {
            "APIKey": AIS_API_KEY,
        ...
        print("Abonnement et filtres envoyés avec succès.", flush=True)
