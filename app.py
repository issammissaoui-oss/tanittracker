...
def on_message(ws, message):
    ...
    try:
        ...
        # Afficher le type de message reçu dans les logs Render pour diagnostic
        msg_type = packet.get("MessageType")
        if msg_type == "SubscriptionConfirmation":
            print("[AIS] Abonnement confirmé avec succès par le serveur.")
            ship_data["status"] = "Connecté (En attente de signal)"
            return
        ...
        # Si on reçoit n'importe quel message, on le logue pour voir si des données affluent
        print(f"[AIS Reçu] Type: {msg_type} | MMSI: {mmsi_str} | Nom: {ship_name}")
        ...
        if TANIT_MMSI in mmsi_str or "TANIT" in ship_name:
            ...
            if lat is not None and lon is not None:
                ...
                print(f"--> POSITION TROUVÉE POUR TANIT : {lat}, {lon}")
...
def on_error(ws, error):
    global ship_data
    ship_data["status"] = "Erreur de connexion"
    print("Erreur WebSocket:", error)

def on_close(ws, close_status_code, close_msg):
    global ship_data
    ship_data["status"] = "Déconnecté, reconnexion..."
    print(f"Fermeture WebSocket: {close_status_code} - {close_msg}")

def run_ais_stream():
    def on_open(ws):
        print("Ouverture du socket WebSocket, envoi de l'abonnement...")
        # Utilisation d'une boîte englobante très large sur toute la Méditerranée
        sub = {
            "APIKey": AIS_API_KEY,
...
# Lancement du thread d'écoute AIS
