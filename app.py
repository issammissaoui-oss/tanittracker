...
def on_message(ws, message):
    ...
    try:
        ...
        # On accepte les messages de position
        ...
        # Vérification par MMSI
        if mmsi == TANIT_MMSI or str(TANIT_MMSI) in str(packet):
...
def on_error(ws, error):
    global ship_data
    ship_data["status"] = "Erreur réseau"
...
def run_ais_stream():
    def on_open(ws):
        global ship_data
        ship_data["status"] = "Flux ouvert"
        # Souscription globale large sur toute la Méditerranée et l'Europe du Sud
        sub = {
            "APIKey": AIS_API_KEY,
