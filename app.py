...
AIS_API_KEY = "e2e419484ce3bd746896a04b950fee53dfd909dcb872bf07bbdb910ef8fad6da"
TANIT_MMSI = "672748000"

# Position initiale (Port de La Goulette ou dernière position connue)
ship_data = {
    "mmsi": TANIT_MMSI,
    "name": "C/F TANIT",
    "latitude": 36.8170,  
    ...
    "speed": 0.0,
    "course": 0.0,
    "timestamp": "En attente de signal...",
    "packets": 0,
    "status": "Recherche du signal AIS..."
}
...
def on_message(ws, message):
    ...
    try:
        ...
        if msg_type == "SubscriptionConfirmation":
            ship_data["status"] = "Connecté au flux AIS (En écoute)"
            print("[AIS] Connecté avec succès au serveur WebSocket.")
            return
        ...
        # Afficher dans la console tous les paquets reçus pour voir ce qui passe
        print(f"[AIS REÇU] MMSI: {mmsi_str} | Nom: {ship_name} | Type: {msg_type}")

        # On élargit la condition pour capturer le Tanit par MMSI ou par son nom
        if TANIT_MMSI in mmsi_str or "TANIT" in ship_name:
            ...
            if lat is not None and lon is not None:
                ...
                ship_data["packets"] += 1
                ship_data["status"] = "En direct (Signal capté)"
                print(f"--> [TROUVÉ] C/F TANIT -> Lat: {lat}, Lon: {lon}")
    except Exception as e:
        print("Erreur de traitement du paquet:", e)
...
def run_ais_stream():
    def on_open(ws):
        # On cible une zone englobant la Tunisie (large bounding box) SANS filtrer uniquement par MMSI au début pour tester si des données arrivent
        sub = {
            "APIKey": AIS_API_KEY,
            "BoundingBoxes": [[[35.0, 8.0], [38.0, 12.0]]],
            "FilterMessageTypes": ["PositionReport", "StandardClassBPositionReport"]
        }
        ws.send(json.dumps(sub))
        print("Abonnement envoyé pour la zone de Tunis/La Goulette.")
...
<body>
    ...
    <script>
        var map = L.map('map').setView([36.8170, 10.3050], 15);
